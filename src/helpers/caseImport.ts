...
export function createCaseDatabaseRow(
  ...
  return {
    ...
    loan_account_number: item.accountNo,

    outstanding_amount:
      item.pendingAmount || item.loanAmount,
    ...
    remarks: [
      `Bank: ${item.bankName || "Not Available"}`,
      `Branch: ${item.branchName || "Not Available"}`,
      `Loan Type: ${item.loanType || "Recovery"}`,
      `Scheme Code: ${item.schemeCode || "Not Available"}`,
      `Account Segment: ${item.accountSegment || "Not Available"}`,
      `Asset Classification: ${item.assetClassification || "Not Available"}`,
      `Sanction Limit: ${item.sanctionLimit || 0}`,
      `Customer Balance: ${item.customerBalance || 0}`,
