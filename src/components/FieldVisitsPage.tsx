...
export default function FieldVisitsPage() {
  ...
  `}</style>
    ...
    <section className="fv-panel"><div className="fv-filters"><input className="fv-input" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Customer, account, executive, remark..."/><select className="fv-input" value={outcome} onChange={e=>setOutcome(e.target.value)}>{outcomes.map(x=><option key={x}>{x}</option>)}</select></div><div className="fv-wrap"><table className="fv-table"><thead><tr><th>Visit Time</th><th>Customer / Account</th><th>Executive</th><th>Area</th><th>Outcome</th><th>Remark</th><th>Follow-up</th><th>Proof</th></tr></thead><tbody>{filtered.map(r=><tr key={r.id}><td>{new Date(r.captured_at).toLocaleString("en-IN")}</td><td><b>{r.customer}</b><br/>{r.account}</td><td>{r.executive}</td><td>{r.area}</td><td><span className="fv-badge">{r.outcome||"Not recorded"}</span></td><td>{r.remarks||"-"}</td><td>{r.next_follow_up||"-"}</td><td><button className="fv-btn" onClick={()=>void openProof(r)}>View</button></td></tr>)}{!loading&&filtered.length===0&&<tr><td colSpan={8}>Abhi matching visit record nahi hai.</td></tr>}</tbody></table></div></section>
