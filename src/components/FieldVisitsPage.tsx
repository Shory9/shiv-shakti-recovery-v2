...
type BankCode = "BOB" | "SBI";
type BankFilter = "All" | BankCode;
type TaggedVisit = Visit & { bankCode:BankCode };
type CaseRow = { id:string; account_number?:string|null; account_name?:string|null; bank_name?:string|null; bankCode:BankCode };
type Executive = { id:string; executive_code?:string|null; full_name?:string|null; area?:string|null; bankCode:BankCode };
type VisitView = TaggedVisit & { customer:string; account:string; bank:string; executive:string; area:string };
...
export default function FieldVisitsPage() {
  ...
  const load=useCallback(async()=>{
    ...
    try {
      const [v,c,e,sv,sc,se]=await Promise.all([
        ...
        supabase.from("sbi_field_visits").select("*").order("captured_at",{ascending:false}),
        supabase.from("sbi_cases").select("id,account_number,account_name,bank_name"),
        supabase.from("sbi_executives").select("id,executive_code,full_name,area")
      ]);
      if(v.error) throw v.error; if(c.error) throw c.error; if(e.error) throw e.error; if(sv.error) throw sv.error; if(sc.error) throw sc.error; if(se.error) throw se.error;
      setVisits([...(v.data??[]).map(row=>({...row,bankCode:"BOB" as const})),...(sv.data??[]).map(row=>({...row,bankCode:"SBI" as const}))] as TaggedVisit[]);
      setCases([...(c.data??[]).map(row=>({...row,bankCode:"BOB" as const})),...(sc.data??[]).map(row=>({...row,bankCode:"SBI" as const}))] as CaseRow[]);
      setExecutives([...(e.data??[]).map(row=>({...row,bankCode:"BOB" as const})),...(se.data??[]).map(row=>({...row,bankCode:"SBI" as const}))] as Executive[]);
  ...
  const rows=useMemo<VisitView[]>(()=>{
    const cm=new Map(cases.map(c=>[`${c.bankCode}:${c.id}`,c]));
    const em=new Map(executives.map(e=>[`${e.bankCode}:${e.id}`,e]));
    return visits.map(v=>{const c=cm.get(`${v.bankCode}:${v.case_id}`);const e=em.get(`${v.bankCode}:${v.executive_id}`);return {...v,customer:c?.account_name||"Unknown",account:c?.account_number||"-",bank:c?.bank_name||(v.bankCode==="SBI"?"State Bank of India":"Bank of Baroda"),executive:`${v.bankCode} · ${e?.executive_code||""}${e?.executive_code?" - ":""}${e?.full_name||"Unknown"}`,area:e?.area||"-"};});
  },[visits,cases,executives]);

  const filtered=useMemo(()=>{const q=search.trim().toLowerCase();return rows.filter(r=>(bankFilter==="All"||r.bankCode===bankFilter)&&(outcome==="All"||(r.outcome||"Not recorded")===outcome)&&(!q||[r.customer,r.account,r.executive,r.area,r.remarks||""].some(x=>x.toLowerCase().includes(q))));},[rows,search,outcome,bankFilter]);
  ...
  async function openProof(row:VisitView){
    ...
    const {data,error:photoError}=await supabase.storage.from(row.bankCode==="SBI"?"sbi-visit-photos":"visit-photos").createSignedUrl(row.photo_path,3600);
  ...
  `}</style>
    ...
    <section className="fv-panel"><label><b>Bank Filter</b><select className="fv-input" style={{width:"100%",marginTop:8}} value={bankFilter} onChange={e=>setBankFilter(e.target.value as BankFilter)} aria-label="Filter visits by bank"><option value="All">All Banks</option><option value="BOB">Bank of Baroda</option><option value="SBI">State Bank of India</option></select></label></section>
